...
export function PortfolioManager() {
  ...
  const loadData = useCallback(async () => {
    try {
      const [{ data: tattoosData }, { data: categoriesData }] =
        await Promise.all([
          supabase
            .from("tattoos")
            .select(
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editingId) {
        ...
        setSuccess("Tatuagem atualizada com sucesso!")
      } else {
        ...
        setSuccess("Tatuagem adicionada com sucesso!")
      }
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      ...
      setSuccess("Tatuagem removida com sucesso!")
      await loadData()
      setTimeout(() => setSuccess(""), 3000)
    } catch (err) {
      console.error("Error deleting tattoo:", err)
      setError("Erro ao remover tatuagem")
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-gray-400 mt-1">
            Adicione, edite ou remova tatuagens do catálogo
          </p>
        </div>
        <button
          onClick={() => setShowForm(!showForm)}
          className="bg-gold text-dark-bg px-6 py-3 rounded-lg font-semibold hover:bg-gold-dark transition-colors flex items-center gap-2"
        >
          {showForm ? (
            <>
              <X className="w-5 h-5" />
      ...
      {showForm && (
        <div className="bg-dark-secondary rounded-lg border border-gold/20 p-6">
          <h3 className="text-xl font-bold text-white mb-4">
            {editingId ? "Editar Tatuagem" : "Nova Tatuagem"}
          </h3>
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              ...
              <textarea
                ...
                rows={3}
                className="w-full px-4 py-2 border border-gold/20 rounded-lg bg-dark-bg text-white placeholder-gray-500 focus:ring-2 focus:ring-gold focus:border-transparent"
                placeholder="Descreva os detalhes da tatuagem..."
              />
            </div>
              ...
              <div className="space-y-3">
                {imagePreview ? (
                  <div className="relative w-64 mx-auto">
                    <div className="aspect-square rounded-lg overflow-hidden border-2 border-gold/20">
                      <img
                        src={imagePreview}
                        alt="Preview"
                  ...
                  <div className="border-2 border-dashed border-gold/20 rounded-lg p-8 text-center hover:border-gold/50 transition-colors">
                ...
                <input
                  ...
                  className="w-full px-4 py-2 border border-gold/20 rounded-lg bg-dark-bg text-white file:mr-4 file:py-2 file:px-4 file:rounded-lg file:border-0 file:bg-gold file:text-dark-bg file:font-semibold hover:file:bg-gold-dark file:cursor-pointer cursor-pointer"
            ...
            <div className="flex items-center gap-2">
              <input
                ...
                className="w-4 h-4 text-gold bg-dark-bg border-gold/20 rounded focus:ring-gold"
              />
            ...
            <div className="flex gap-3">
              <button
                type="submit"
                disabled={uploading}
                className="bg-gold text-dark-bg px-6 py-2 rounded-lg font-semibold hover:bg-gold-dark transition-colors flex items-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed"
              >
                {uploading ? (
                  <>
              ...
              <button
                ...
                onClick={resetForm}
                disabled={uploading}
                className="border border-gold/20 text-white px-6 py-2 rounded-lg font-semibold hover:bg-dark-bg transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
              >
                Cancelar
              </button>
            </div>
      ...
      <div className="bg-dark-secondary rounded-lg border border-gold/20 overflow-hidden">
        <div className="p-4 border-b border-gold/20">
          <h3 className="text-lg font-semibold text-white">
            Tatuagens no Portfólio ({tattoos.length})
          </h3>
        </div>
            ...
            <tbody className="divide-y divide-gold/10">
              {tattoos.length === 0 ? (
                <tr>
                  ...
                  >
                    Nenhuma tatuagem cadastrada. Clique em "Adicionar Tatuagem"
                    para começar.
                  </td>
                </tr>
              ) : (
                tattoos.map((tattoo) => (
                  <tr key={tattoo.id} className="hover:bg-dark-bg/50">
