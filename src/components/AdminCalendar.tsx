...
export function AdminCalendar() {
  ...
  const getDayColor = (count: number) => {
    if (count === 0) return "bg-dark-bg"
    if (count <= 5) return "bg-green-900/40 border-green-500"
    if (count <= 10) return "bg-yellow-900/40 border-yellow-500"
        ...
        <div className="bg-dark-secondary rounded-lg border border-gold/20 p-4 sm:p-6">
          <div className="flex items-center justify-between mb-4 sm:mb-6">
            ...
            <div className="flex space-x-2">
              <button
                ...
                className="p-2 hover:bg-dark-bg rounded-lg transition-colors text-white"
              >
          ...
          <div className="mt-4 sm:mt-6 flex items-center justify-center space-x-3 sm:space-x-6 text-xs sm:text-sm flex-wrap gap-2">
            ...
            <div className="flex items-center space-x-2">
              ...
              <span className="text-gray-400">Até 5</span>
            </div>
            <div className="flex items-center space-x-2">
              <div className="w-4 h-4 bg-yellow-900/40 border border-yellow-500 rounded"></div>
              <span className="text-gray-400">5-10</span>
            </div>
            <div className="flex items-center space-x-2">
              <div className="w-4 h-4 bg-red-900/40 border border-red-500 rounded"></div>
              <span className="text-gray-400">Acima de 10</span>
            </div>
          </div>
        </div>
      </div>
            ...
            dayAppointments.length === 0 ? (
              ...
              <div className="space-y-3">
                {dayAppointments.map((appointment) => (
                  <div
                    key={appointment.id}
                    className="border border-gold/20 rounded-lg p-3 bg-dark-bg"
                  >
